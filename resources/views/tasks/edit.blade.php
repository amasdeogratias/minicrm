                ...
                <a href="{{route('tasks.index')}}" class="underline mb-">All Tasks</a>
                    <form method="POST" action="{{ route('tasks.update', $task) }}">
                        ...
                        <div class="mt-4">
                            ...
                            <textarea id="description" class="block mt-1 w-full border-gray-300 focus:border-indigo-500" name="description" required autofocus autocomplete="name">{{old('description', $task->description)}}</textarea>
                        ...
                        <div class="mt-4">
                            ...
                            <select class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500" name="status">
                                @foreach (\App\Enums\TaskStatus::cases() as $status )
                                    <option value="{{ $status->value }}" @selected(old('status', $task->status) == $status->value)>{{ $status->value }}</option>
                                @endforeach
                            </select>
